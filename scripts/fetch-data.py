...
class RiksdataFetcher:
    ...
    def fetch_ssb_data(self):
        ...
        # Start with the most important datasets first
        priority_datasets = [
        ...
        logger.info(f"Fetching {len(priority_datasets)} SSB datasets...")
        ...
        for dataset in priority_datasets:
            try:
                ...
                if failed_fetches > 10:
                    logger.warning("Too many failures, stopping SSB fetch to avoid rate limiting")
                    break
        
        logger.info(f"SSB fetch completed: {successful_fetches} successful, {failed_fetches} failed")
